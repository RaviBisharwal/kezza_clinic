...
2. SKIN         → Dr. Amrita Makhija / Dr. Neelam Choudhary (9216063686)
3. ANTI-AGING   → Dr. Amrita Makhija / Dr. Neelam Choudhary (9216063686)
...
`;
    ...
    let state = {
        ...
        // Keeps a submitted booking distinct from an active intake flow.
        completedConsultation: null
    };
    ...
    const DEPARTMENT_ROUTING_TABLE = {
        ...
        SKIN: {
            ...
            departmentName: 'Skin Team (Dr. Amrita Makhija / Dr. Neelam Choudhary)',
            phone: '9216063686',
            isTransplant: false,
            isDental: false,
...
— Kezza AI`;
    ...
    async function handleConsultationFlow(userText, lang) {
        ...
        switch (flow.state) {
            ...
            case CONSULTATION_STATES.REVIEW: {
                const isConfirm = [
                    '✅ confirm & send', 'confirm & send', 'confirm and send',
                    '✅ submit consultation', 'submit consultation', 'submit request',
                    '✅ haan, send karein', '✅ yes, submit request', '✅ yes, send',
                    'confirm', 'submit', 'send', 'yes', 'haan', 'ha', 'proceed',
                    'theek hai', 'sahi hai', 'bhejo', 'done', 'ok', 'okay'
                ].some(k => lower === k || lower === '✅ ' + k) ||
                lower.startsWith('confirm ') || lower.startsWith('submit ');
    ...
    async function submitConsultationAuto(data, lang) {
        ...
        const GOOGLE_SHEET_URL = 'https://script.google.com/macros/s/AKfycbzT9bA1OTFH4Ox6k-0ROsNYCxtdbwT8VLVzjAODHl3S3ZAxvxxTMj39uPilgcs4NgvJuQ/exec';
...
📱 <strong>WhatsApp:</strong> ${escapeHtml(data.phone || '')}
        ...
        // Mark this intake complete before showing the final quick replies.
        state.completedConsultation = { id: consultationId };
        ...
        const fallbackMsg = (lang === 'hinglish')
            ? `✅ <strong>Consultation Details Ready!</strong>\n\n${summaryCard}\n\n${afterHoursNote}<p style="font-size:13px;color:#059669;margin:8px 0;"><strong>📲 WhatsApp auto-redirect ho raha hai...</strong> Agar automatically open nahi hua, toh neeche button par tap karein:</p>\n\n<a href="${waUrl}" target="_blank" class="kezza-whatsapp-btn"><i class="fab fa-whatsapp"></i> WhatsApp par Send Karein (${routing.phone})</a>`
            : ((lang === 'hindi')
                ? `✅ <strong>Consultation Details तैयार हैं!</strong>\n\n${summaryCard}\n\n${afterHoursNote}<p style="font-size:13px;color:#059669;margin:8px 0;"><strong>📲 WhatsApp खुल रहा है...</strong> यदि स्वतः न खुले तो नीचे बटन पर क्लिक करें:</p>\n\n<a href="${waUrl}" target="_blank" class="kezza-whatsapp-btn"><i class="fab fa-whatsapp"></i> WhatsApp पर Send करें (${routing.phone})</a>`
                : `✅ <strong>Consultation Details Ready!</strong>\n\n${summaryCard}\n\n${afterHoursNote}<p style="font-size:13px;color:#059669;margin:8px 0;"><strong>📲 Opening WhatsApp...</strong> If it did not open automatically, tap the button below:</p>\n\n<a href="${waUrl}" target="_blank" class="kezza-whatsapp-btn"><i class="fab fa-whatsapp"></i> Send via WhatsApp (${routing.phone})</a>`);
...
👨‍⚕️ Specialist: Dr. Amrita Makhija / Dr. Neelam Choudhary
    ...
    <div style="margin-top:14px;padding:12px 14px;background:#f8fafc;border-radius:10px;border:1.5px solid #cbd5e1;">
        ...
        <div style="padding:10px 12px;background:#dcfce7;border-radius:8px;margin-bottom:8px;">
            ...
            <div style="font-size:12.5px;color:#1e293b;line-height:1.6;">
                <strong>Specialist:</strong> Dr. Amrita Makhija / Dr. Neelam Choudhary<br>
    ...
    function getLocalPhotoAssessment(textContext, lang) {
        ...
        if (isExplicitBoth) {
            return {
                ...
                specialist: 'Dr. Amrita Makhija / Dr. Neelam Choudhary (Skin) & Dr. Ankit Bhalothia (Hair)',
...
<div class="kezza-photo-card" style="border-left:4px solid #f59e0b;">
    ...
    async function generateLocalResponse(userText) {
        ...
        // Acknowledge the success-card Thank You before intent routing so a completed booking never restarts intake.
        const isThankYou = /^(thank\s*(you|u)|thanks|thx|shukriya|dhanyavaad|dhanyavad|ty)\b/.test(lower);
        if (state.completedConsultation && isThankYou) {
            const id = escapeHtml(state.completedConsultation.id);
            return {
                text: (lang === 'hinglish' || lang === 'hindi')
                    ? `Aapka swagat hai! 😊 Aapki consultation request <strong>${id}</strong> successfully submit ho chuki hai. Humein aapse aur koi details nahi chahiye—hamari team jaldi contact karegi.`
                    : `You're welcome! 😊 Your consultation request <strong>${id}</strong> has already been submitted. We don't need any further details; our team will contact you shortly.`,
                quickReplies: ['📍 Clinic Locations', 'Ask Another Question']
            };
        }
        ...
        if (strictIntent) {
            ...
            if (taxonomy.category === 'SKIN') {
                ...
                if (lang === 'hinglish') {
                    return {
                        text: `Bilkul 😊 <strong>${label}</strong> Kezza ke <strong>Skin treatment</strong> category mein aata hai.\n\nIske liye <strong>Dr. Amrita Makhija</strong> aur <strong>Dr. Neelam Choudhary</strong> ki Skin Specialist team se consultation book kar sakte hain.\n\n📞 <strong>${skinPhone}</strong>\n\n${skinBtn}`,
                        quickReplies: ['📅 Book Skin Consultation', 'Chat with Skin Team', '📍 Clinic Locations']
                    };
                }
                if (lang === 'hindi') {
                    return {
                        text: `बिल्कुल 😊 <strong>${label}</strong> केज़ा के <strong>स्किन ट्रीटमेंट</strong> श्रेणी में आता है।\n\nइसके लिए आप <strong>Dr. Amrita Makhija</strong> और <strong>Dr. Neelam Choudhary</strong> की स्किन विशेषज्ञ टीम से परामर्श ले सकते हैं।\n\n📞 <strong>${skinPhone}</strong>\n\n${skinBtn}`,
                        quickReplies: ['📅 स्किन परामर्श बुक करें', '📍 क्लीनिक लोकेशन']
                    };
                }
                return {
                    text: `Absolutely! <strong>${label}</strong> falls under Kezza's <strong>Skin treatment</strong> category.\n\nConsult our Skin Specialists — <strong>Dr. Amrita Makhija</strong> and <strong>Dr. Neelam Choudhary</strong>.\n\n📞 <strong>${skinPhone}</strong>\n\n${skinBtn}`,
                    quickReplies: ['📅 Book Skin Consultation', 'Chat with Skin Team', '📍 Clinic Locations']
                };
            }
        ...
        // Acne, Scars, Dark Circles, Pigmentation → Skin Team (Dr. Amrita Makhija / Dr. Neelam Choudhary)
        if (DEPARTMENTS.acne_scar.scope.some(k => norm.includes(k))) {
            ...
            if (lang === 'hinglish') {
                return {
                    text: `Acne, acne scars, dark circles ya pigmentation ke liye <strong>Dr. Amrita Makhija</strong> aur <strong>Dr. Neelam Choudhary</strong> ki Skin Specialist team se contact kar sakte hain.\n\n📞 <strong>${spec.phone}</strong>\n\n${btn}`,
                    quickReplies: ['📅 Book Skin Consultation', 'Chat with Skin Team', '📍 Clinic Locations']
                };
            }
            return {
                text: `For acne, acne scars, dark circles, or pigmentation concerns, consult our Skin Specialists — <strong>Dr. Amrita Makhija</strong> and <strong>Dr. Neelam Choudhary</strong>.\n\n📞 <strong>${spec.phone}</strong>\n\n${btn}`,
                quickReplies: ['📅 Book Skin Consultation', 'Chat with Skin Team', '📍 Clinic Locations']
            };
        }
        ...
        // Laser Treatments → Skin Team (Dr. Amrita Makhija / Dr. Neelam Choudhary)
        if (norm.includes('laser')) {
            ...
            if (lang === 'hinglish') {
                return {
                    text: `Laser treatment ke liye <strong>Dr. Amrita Makhija</strong> aur <strong>Dr. Neelam Choudhary</strong> ki Skin Specialist team available hai.\n\n📞 <strong>${spec.phone}</strong>\n\n${btn}`,
                    quickReplies: ['📅 Book Laser Consultation', 'Chat with Skin Team', '📍 Clinic Locations']
                };
            }
            return {
                text: `For laser treatments, our Skin Specialists — <strong>Dr. Amrita Makhija</strong> and <strong>Dr. Neelam Choudhary</strong> — are available.\n\n📞 <strong>${spec.phone}</strong>\n\n${btn}`,
                quickReplies: ['📅 Book Laser Consultation', 'Chat with Skin Team', '📍 Clinic Locations']
            };
        }

        // Anti-Aging (Wrinkles, Tightening, HIFU) → Skin Team (Dr. Amrita Makhija / Dr. Neelam Choudhary)
        if (DEPARTMENTS.anti_aging.scope.some(k => norm.includes(k))) {
            ...
            if (lang === 'hinglish') {
                return {
                    text: `Wrinkle reduction aur skin tightening treatment ke liye <strong>Dr. Amrita Makhija</strong> aur <strong>Dr. Neelam Choudhary</strong> ki Skin Specialist team se contact kar sakte hain.\n\n📞 <strong>${spec.phone}</strong>\n\n${btn}`,
                    quickReplies: ['📅 Book Skin Consultation', 'Chat with Skin Team', '📍 Clinic Locations']
                };
            }
            return {
                text: `For wrinkle reduction and skin tightening treatments, our Skin Specialists — <strong>Dr. Amrita Makhija</strong> and <strong>Dr. Neelam Choudhary</strong> — are available.\n\n📞 <strong>${spec.phone}</strong>\n\n${btn}`,
                quickReplies: ['📅 Book Skin Consultation', 'Chat with Skin Team', '📍 Clinic Locations']
            };
        }
