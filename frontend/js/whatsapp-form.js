...
(function() {
    ...
    // Background lead dispatch. Posts to the backend when one is available, and only
    // falls back to the Google Sheets webhook when it is not — so a lead is never
    // written to the sheet twice.
    async function sendLead(payload) {
        ...
        } catch (apiErr) { /* backend unavailable — fall through */ }
        ...
        try {
            ...
            console.warn('[Kezza] Lead sync failed:', sheetErr);
        }
    }
...
👨‍⚕️ *Routed to:* ${deptName}`;
            ...
            const leadPayload = {
                ...
                Department: deptName,
                'Department': deptName,
                concern: message || 'General Consultation',
                Concern: message || 'General Consultation',
                'Concern / Duration': message || 'General Consultation',
                'concern / duration': message || 'General Consultation',
                'Concern/Duration': message || 'General Consultation',
            ...
            // Open WhatsApp SYNCHRONOUSLY, while the browser still treats this as a
            // user gesture. Awaiting a network call first causes popup blockers to
            // silently drop window.open, so the lead sync below is fire-and-forget.
            try {
            ...
            // Deduplicated safe sync (background, never blocks the WhatsApp hand-off):
            // Try /api/lead first. If the backend is running it saves the lead and syncs
            // to Google Sheets. If the backend is unavailable (static hosting), fall back
            // to posting the Google Sheets webhook directly — exactly one of the two runs.
