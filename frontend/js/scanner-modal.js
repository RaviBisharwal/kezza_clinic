...
(function () {
      ...
      <div class="ks-modal-card" id="ksModalCard">
        ...
        <div class="ks-body" id="ksModalBody">
          ...
          <div class="ks-stage ks-active" id="ksStageScan">
            ...
            <!-- Primary Analyze / Proceed Button -->
            <button type="button" class="ks-btn-primary" id="ksBtnAnalyze" style="display:none; margin: 12px 0 8px; width: 100%;" disabled>
            ...
            <div class="ks-details-wizard">
              ...
              <div class="ks-q-stage" data-q="3">
                ...
                <div class="ks-chips-grid" style="margin-top:12px">
                  ...
                  <button type="button" class="ks-chip-btn" data-city="Ajmer"><i class="fas fa-location-dot"></i> Ajmer</button>
              ...
              <div class="ks-q-stage" data-q="4">
                ...
                <div class="ks-clinic-cards-grid">
                  ...
                  <div class="ks-clinic-card" data-clinic="Ajmer">
                    <span class="ks-clinic-card-badge">New Branch</span>
                    <div class="ks-clinic-card-title">Ajmer</div>
                    <div class="ks-clinic-card-loc">Oasis Complex, Shastri Nagar</div>
                  </div>
                </div>
              </div>
          ...
          <div class="ks-stage" id="ksStageResults">
            <div class="ks-results-wrap">
              ...
              <div class="ks-stage-disclaimer" style="font-size: 11.5px; line-height: 1.45; color: #a1a1aa; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); border-radius: 8px; padding: 10px 12px; margin-top: 12px; text-align: center;">
                <i class="fas fa-shield-alt" style="color: #00AFC0; margin-right: 4px;"></i>
                This AI analysis is for preliminary guidance only and is not a medical diagnosis. Please consult a qualified doctor for a proper evaluation.
              </div>
  ...
  function closeModal(force = false) {
    ...
    // Cancel any running countdown or auto-proceed timer
    cancelCountdown();
    if (state.autoProceedTimer) {
      clearTimeout(state.autoProceedTimer);
      state.autoProceedTimer = null;
    }
  ...
  function setupScanStepEvents() {
    ...
    fileInput.addEventListener('change', function () {
      ...
      this.value = ''; // allow selecting same file again if retaken
    });
    ...
    btnRetake.addEventListener('click', () => {
      ...
      const badge = document.querySelector('.ks-preview-badge');
      if (badge) badge.innerHTML = '<i class="fas fa-check-circle"></i> Photo Ready';
      startCameraStream();
    });
  ...
  function displayCapturedImage() {
    ...
    // 1. Reveal and enable the Proceed button immediately
    if (btnAnalyze) {
      btnAnalyze.style.display = 'flex';
      btnAnalyze.disabled = false;
      btnAnalyze.innerHTML = '<span>Proceed to AI Analysis</span> <i class="fas fa-arrow-right"></i>';
    }

    // 2. Clear progress feedback
    if (subtitle) {
      subtitle.innerHTML = 'Photo ready! Starting AI analysis… <span style="color:#00AFC0;cursor:pointer;text-decoration:underline;margin-left:4px;" id="ksInstantProceed">Tap to proceed now</span>';
      const instantLink = document.getElementById('ksInstantProceed');
      if (instantLink) {
        instantLink.addEventListener('click', () => {
          if (state.autoProceedTimer) {
            clearTimeout(state.autoProceedTimer);
            state.autoProceedTimer = null;
          }
    ...
    if (badge) {
      badge.innerHTML = '<i class="fas fa-check-circle"></i> Photo Ready • Analyzing...';
    }

    // 3. Smooth auto-advance after 850ms so user has zero friction
    if (state.autoProceedTimer) {
      clearTimeout(state.autoProceedTimer);
    }
    state.autoProceedTimer = setTimeout(() => {
      if (state.capturedImageBase64 && state.activeStage === 1) {
        stopCameraStream();
        setStage(2);
        runAIAnalysis();
      }
  ...
  async function submitLeadAndRevealResults() {
    if (state.isSubmittingLead) return;
    state.isSubmittingLead = true;
    ...
    // Strict clinic normalization: strictly 'Ajmer', 'Sikar', or 'Jaipur'
    let normClinic = 'Jaipur';
    const rawClinic = (state.answers.clinic || '').toLowerCase();
    if (rawClinic.includes('ajmer')) normClinic = 'Ajmer';
    else if (rawClinic.includes('sikar')) normClinic = 'Sikar';
    else if (rawClinic.includes('jaipur')) normClinic = 'Jaipur';

    // Determine assigned doctor based on clinic and category/concern
    const catLower = (state.answers.category || '').toLowerCase();
    const concernLower = (state.answers.concern || '').toLowerCase();
    const isHair = catLower.includes('hair') || concernLower.includes('hair') || concernLower.includes('scalp');
    let assignedDoc = DOCTORS.SKIN;
    if (normClinic === 'Ajmer') {
      assignedDoc = isHair ? DOCTORS.AJMER_HAIR : DOCTORS.AJMER_SPECIALIST;
    } else {
      assignedDoc = isHair ? DOCTORS.HAIR : DOCTORS.SKIN;
    }

    const consultationId = `KEZZA-${new Date().getFullYear()}-${Math.floor(1000 + Math.random() * 9000)}`;

    const catLabel = state.answers.categoryLabel || (isHair ? 'Hair' : 'Skin');
    const durationVal = state.answers.duration || 'Not specified';
    const concernVal = state.answers.concern || catLabel || 'General Assessment';
    const concernDurationVal = (durationVal && durationVal !== 'Not specified') ? durationVal : concernVal;

    // Prepare lead payload matching chatbot lead parity and Google Sheets column headers
    const leadData = {
      ...
      phone: state.answers.whatsapp,
      mobile_number: state.answers.whatsapp,
      'WhatsApp': state.answers.whatsapp,
      age: parseInt(state.answers.age, 10) || state.answers.age || '',
      email: '',
      city: state.answers.city || normClinic,
      patient_city: state.answers.city || normClinic,
      patientLocation: state.answers.city || normClinic,
      'Patient Location': state.answers.city || normClinic,
      clinic: normClinic,
      preferred_clinic: normClinic,
      'Clinic': normClinic,
      category: catLabel,
      treatment_category: catLabel,
      'Category': catLabel,
      treatment: state.answers.treatment || 'Clinical Consultation',
      service: state.answers.treatment || 'Clinical Consultation',
      'Treatment': state.answers.treatment || 'Clinical Consultation',
      department: catLabel,
      Department: catLabel,
      'Department': catLabel,
      department_name: catLabel,
      date: state.answers.date || '',
      preferredDate: state.answers.date || '',
      preferred_date: state.answers.date || '',
      'Preferred Date': state.answers.date || '',
      time: state.answers.time || '',
      preferredTime: state.answers.time || '',
      preferred_time: state.answers.time || '',
      'Preferred Time': state.answers.time || '',
      ...
      concernDuration: concernDurationVal,
      ...
      specialist: assignedDoc.name,
      'Specialist': assignedDoc.name,
      message: `[AI Scanner Modal] Concern: ${state.answers.concern || 'N/A'}${state.answers.symptoms ? ' | Symptoms: ' + state.answers.symptoms : ''}`,
      notes: (state.answers.concern || '') + (state.answers.symptoms ? ' | Symptoms: ' + state.answers.symptoms : ''),
    ...
    try {
      const apiRes = await fetch(API_LEAD, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    if (!serverSynced) {
      try {
        fetch(GOOGLE_SHEETS_WEBHOOK, {
          ...
          body: JSON.stringify(leadData),
          keepalive: true
        }).catch(e => console.warn('[Scanner Modal Direct Sheet Sync Warn]:', e));
      } catch (sheetErr) {
        console.warn('[Scanner Modal Direct Sheet Sync Error]:', sheetErr);
      }
    }
  ...
  function renderFullResults() {
    ...
    // Normalize clinic display
    let clinicDisplay = 'Jaipur';
    const rawClinic = (ans.clinic || '').toLowerCase();
    if (rawClinic.includes('ajmer')) clinicDisplay = 'Ajmer';
    else if (rawClinic.includes('sikar')) clinicDisplay = 'Sikar';
    else if (rawClinic.includes('jaipur')) clinicDisplay = 'Jaipur';
    ...
    document.getElementById('ksResClinic').textContent = clinicDisplay;
    ...
    // Doctor match based on clinic & category/concern
    const concernLower = (ans.concern || '').toLowerCase();
    const catLower = (ans.category || '').toLowerCase();
    const isHair = concernLower.includes('hair') || concernLower.includes('scalp') || catLower.includes('hair');
    const isAjmer = clinicDisplay === 'Ajmer';

    let doc = DOCTORS.SKIN;
    if (isAjmer) {
      doc = isHair ? DOCTORS.AJMER_HAIR : DOCTORS.AJMER_SPECIALIST;
    } else if (isHair) {
      doc = DOCTORS.HAIR;
    ...
    if (docSpec) docSpec.textContent = `${doc.spec} (${clinicDisplay || doc.clinic})`;

    // Configure WhatsApp Button
    const waCTA = document.getElementById('ksBtnWhatsAppCTA');
    if (waCTA) {
      const targetPhone = isAjmer ? '919216088257' : WHATSAPP_NUMBER;
      const lines = [
        ...
        clinicDisplay ? `Preferred clinic: ${clinicDisplay}` : '',
      ...
      waCTA.href = `https://wa.me/${targetPhone}?text=${msg}`;
